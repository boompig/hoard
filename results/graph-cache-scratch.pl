...
foreach $name (@namelist) {
    open G, "> cache-scratch/Results/$name/data";
    $base = 0;
    $scalability{$name} = 0;
    $fragmentation{$name} = 0;
    for (my $i = 1; $i <= $nthread; $i++) {
	open F, "cache-scratch/Results/$name/cache-scratch-$i";
	my $total = 0;
	my $count = 0;
	my $min = 1e30;
	my $max = -1e30;
...
foreach $name (@namelist) {
	...
	print PLOT "\"cache-scratch/Results/$name/data\" title \"$titlename\" with linespoints\n";
    } else {
	print PLOT "\"cache-scratch/Results/$name/data\" title \"$titlename\" with linespoints,";
    }
}
