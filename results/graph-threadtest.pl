...
foreach $name (@namelist) {
    open G, "> threadtest/Results/$name/data";
    $base = 0;
    $scalability{$name} = 0;
    for (my $i = 1; $i <= 8; $i++) {
	open F, "threadtest/Results/$name/threadtest-$i";
	my $total = 0;
	my $count = 0;
	my $min = 1e30;
	my $max = -1e30;
...
foreach $name (@namelist) {
	...
	print PLOT "\"threadtest/Results/$name/data\" title \"$titlename\" with linespoints\n";
    } else {
	print PLOT "\"threadtest/Results/$name/data\" title \"$titlename\" with linespoints,";
    }
}
