...
/* October 2014: Modified by Angela Demke Brown for use in CSC469 A2 */
...
main (int argc, char *argv[])
{
  ...
  executionTime = (double *) mm_malloc (sizeof(double) * thread_count);
  if (executionTime == NULL) {
	  printf("Failed to allocate %ld bytes for executionTime. Exiting.\n",
		 sizeof(double));
	  exit(1);
  }
...
run_test (void * arg)
{
...

  {
    ...
    if (buf == NULL) {
	    printf("Failed to allocated %ld bytes for buf in thread %d. Exiting\n",
